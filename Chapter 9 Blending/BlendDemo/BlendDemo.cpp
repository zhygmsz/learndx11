...
: D3DApp(hInstance), mLandVB(0), mLandIB(0), mWavesVB(0), mWavesIB(0), mBoxVB(0), mBoxIB(0), mCylinderVB(0), mCylinderIB(0), mGrassMapSRV(0), mWavesMapSRV(0), mBoxMapSRV(0),
  ...
  mTheta(1.3f*MathHelper::Pi), mPhi(0.4f*MathHelper::Pi), mRadius(80.0f), BoltAniMaxFrame(30), mCurFrame(-1), mfTimer(0.0f), SecPerFrame(1.0f / 10), mBoltAniMapSRV(0), mDiffuseMapSRV(0), mCylinderIndexCount(0)
{
...
BlendApp::~BlendApp()
{
	...
	for (int idx = 1; idx <= BoltAniMaxFrame; ++idx)
	{
...
bool BlendApp::Init()
{
	...
	for (int idx = 1; idx <= BoltAniMaxFrame; ++idx)
	{
		fileName = Str2Wstr(format("../../Chapter 10 Stenciling/BoltAnim/Bolt%03d.bmp", idx));
		HR(D3DX11CreateShaderResourceViewFromFile(md3dDevice,
			fileName.c_str(), 0, 0, &mBoltAniMapSRV[idx - 1], 0));
	}
...
void BlendApp::BuildCylinderGeometryBuffers()
{
	...
	GeometryGenerator geoGen;
	geoGen.CreateCylinder(1.0f, 1.0f, 2.0f, 20, 20, cylinder);
